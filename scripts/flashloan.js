...
async function startFlashLoan() {
  ...
  deployer=accounts[0]
  player=accounts[1]
  const flashLoanContract = await ethers.getContract("simpleFlashLoan")
  flashLoan = flashLoanContract.connect(deployer)
  // console.log(network,flashLoanContract.address)
  // let data=await flashLoan.test()
  const entranceFee = await flashLoan.startFlashLoan("0xDF1742fE5b0bFc12331D8EAec6b478DfDbD31464",BigInt(10000 * 10**18))
  // console.log("Entered!")
}
